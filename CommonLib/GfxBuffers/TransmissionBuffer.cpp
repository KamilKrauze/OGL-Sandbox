...
void TransmissionBuffer::Create()
{
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, lightDepthMap, 0);
...
void TransmissionBuffer::BindData(uint32_t& shader, const int32_t& shadowMapBinding, const int32_t& transmissionMapBinding, const int32_t& lightDepthMapBinding)
{
    if ((shadowMapBinding == transmissionMapBinding) == lightDepthMapBinding)
    {
    ...
    glActiveTexture(GL_TEXTURE0 + lightDepthMapBinding); // Index from 0.
    glBindTexture(GL_TEXTURE_2D, lightDepthMap);
    glUniform1i(glGetUniformLocation(shader, "LightDepthMap"), lightDepthMapBinding);
}
...
void TransmissionBuffer::Update(const glm::vec3& light_pos)
{
    ...
    glUniform1fv(glGetUniformLocation(shaderProgram, "NearPlane"), 1, &near_plane);
    glUniform1fv(glGetUniformLocation(shaderProgram, "FarPlane"), 1, &far_plane);
